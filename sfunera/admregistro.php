...
$ced = "";
if ($rot) {
	$ced = (isset($_REQUEST["ced"])) ? trim($_REQUEST["ced"]) : "";
} else {
...
if ( !empty($ced) ) {
  ...
  $botad = (isset($_REQUEST["botad"])) ? $_REQUEST["botad"] : "";
    ...
    $.ajax({
		...
		$("#fecdia").val(res.fecact);   $("#deupre").val(res.deupre);
			...
			if (cdval == "") cdval = 0;
                ...
                console.log("done-p: " + JSON.stringify(res));
        }
...
//                      $("#calcuo").html( msjcal );
                	...
                	console.log("done-c: " + JSON.stringify(res));
                }
          });
        }
    });
...
//	   if ($gdir == 0) $errpri = "registro de dirección de asociado";
   ...
   if ( file_exists($epdf) ) {
	...
	if ($botad == "eli") {
		// eliminar archivo pdf de usuario
		@unlink( $epdf );
	}
//echo $updf;
//   } else {
...
if($rot) {  // sesión admin
?>
   <form name="f1" action="<?php echo $url; ?>" method="post">
        <table width="100%" height="100%" border="0" cellspacing="0">
          <tr>
                <td width="25%" align="center">
                        <img src="<?php echo JURI::base(); ?>images/stories/capunefm/iconcapunefm.png" />
                </td>
                <td width="50%" align="center">
                        <h3>GESTOR DE PROGRAMA</h3>
                        <br />
                        <b>ASOCIADO <?php echo strtoupper( $app->getCfg("MetaKeys") ); ?></b>
                </td>
                <td width="25%" valign="bottom" align="right">
                <?php echo JHTML::_("date", "now", JText::_("DATE_FORMAT_LC")) . "<br>" . date("h:i:s a"); ?>
                </td>
          </tr>
          <tr>
                <td colspan="3" align="center">
                <hr width="95%">
                        ID: <input type="text" name="ced" size="20" value="">&nbsp;&nbsp;
			<input type=submit name="botad" value="con" size="12">
                <hr width="95%">
                </td>
          </tr>
          <tr>
                <td colspan="3" align="center">
                <hr width="95%">Cantidad de registros procesados:
		<?php echo count(glob("/srv/www/htdocs/DEF_" . $vlib->getCredito() . "_*.TXT")); ?>
                <hr width="95%">
                </td>
          </tr>
          <tr>
                <td colspan="3" align="center">
                <hr width="95%">Eliminar pdf genado por usuario con error de registro, clic en boton:
                    <input type="submit" name="botad" value="eli" onclick="if(!confirm('¿Seguro de borrar pdf de asociado?')){return false;}">
                <hr width="95%">
                </td>
          </tr>
          <tr>
                ...
                <hr width="95%">Recuperar pdf regenado por administrador procesa, clic en boton:
                                <input type="submit" name="botad" value="rec">
                <hr width="95%">
                </td>
          </tr>
          <tr>
                <td colspan="3" align="center">
                <hr width="95%">Recuperar pdf regenado por administrador procesa, clic en boton:
                                <input type="submit" name="botad" value="fma">
                <hr width="95%">
                </td>
          </tr>
        </table>
