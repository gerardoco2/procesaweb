...
//      mk += '<td><input name="apegf' + i + '" id="apegf' + i + '" type="text" class="form-control input-sm" size="12" placeholder="Apellido">';
    ...
    $.ajax({
        ...
        if(res.tokfun != currSess){
                ...
                $("#tipsoc").val(res.tipper);   $("#afilia").val(res.afilia);
...
<div id="reg" class="fila">
    ...
    <form role="form" name="sfuner" id="sfuner" method="post" action="<?php echo $rou; ?>" enctype="multipart/form-data">
		...
		<p class="text-warning">Valide los datos del titular contratante</p>
			...
			<input type="text" name="afilia" id="afilia" readonly class="form-control sinborde hidden">
                    </div>
                </div>
		...
		<p class="text-warning">Ingrese los datos del único beneficiario por fallecimiento del titular para pago de la cláusula</p>
		    ...
		    <div class="col-sm-6">
                   	...
                   	<input name="nomben" id="nomben" type="text" required class="form-control" placeholder="Nombre y apellido del beneficiario" />
                    </div>
		    <div class="col-sm-6">
                    	<label class="control-label">Cédula</label>
                   	<input name="cedben" id="cedben" type="text" required class="form-control" placeholder="Número de cédula" />
                    </div>
                </div>
                <div class="form-group row">
		    <div class="col-sm-6">
                    	<label class="control-label">Teléfono</label>
                   	<input name="telben" id="telben" type="text" required class="form-control" placeholder="Teléfono del beneficiario" />
                    </div>
		    <div class="col-sm-6">
                    	<label class="control-label">Correo</label>
                   	<input name="corben" id="corben" type="email" required class="form-control" title="Ingrese un correo válido" placeholder="Ej: correo@dominio.com" />
                    </div>
                </div>
                <div class="form-group row">
		    <div class="col-sm-12">
		    	...
		    	<p class="form-control-static">
	                   <textarea name="dirben" id="dirben" required class="form-control" rows="2" placeholder="Dirección del beneficiario"></textarea>
			</p>
		    </div>
                </div>
...
//        $cont .= $dirsoc . PHP_EOL;
        ...
        $cn = $vlib->guardarTXT($rdir, $cont);
        $ln = explode("\n", $cn);

//      if ( file_exists($file_socio) && count($ln) > 0 ) 
	if ( count($ln) > 0 ) {
//              unlink( $file_socio );

           $vlib->actDirUsuario($ced, $dirsoc);

           $fpdf = $vlib->generarpdf($ced, $docr, $monafi);
           if ( file_exists($fpdf) ) {
...
?>
	<div class="alert alert-danger">
	   <strong>Error en planillas!</strong> Reingrese a la sesión de asociado o contacte al administrador
	</div>
<?php
	}
?>
