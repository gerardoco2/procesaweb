...
//		exec("rm " . $file_socio);
	...
	$ejec = exec($raiz . $rdir . "ejec_pvx 2>&1");
//	$ejec = shell_exec("/home/pvxfull/pvx /home/pvx/procesa/RU_VERIF"); 
//	$ejec = exec("/home/pvxfull/pvx /home/pvx/procesa/RU_VERIF>/dev/null");
	...
	if ( file_exists($filas) )
	{
		...
		if ( count($dbasico) > 0 )
		{
			...
			{
				$r = stristr($t3, $o3);
				if ( $r )  $afianzado[$t3] = $l3;
			}
			foreach ($cn AS $l4 => $t4)
			{
				$r = stristr($t4, $o4);
				if ( $r )  $disponible[$t4] = $l4;
			}
				...
				if ( $moncuota >= 0 ) {
					echo "
					  <tr $bgp>
						...
						<td align=right>" . number_format(abs($monprest), 2, "," ,".") . "&nbsp;</td>
<!--
						<td align=right>$nrcuotas</td>
						<td align=right>" . number_format($moncuota, 2, "," ,".") . "</td>
-->
						<td align=right>" . number_format(abs($salprest), 2, "," ,".") . "</td>
					  </tr>       
					";
