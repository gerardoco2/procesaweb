...
require_once("lib_credinavidad.php");
$ver = new Libcredinavidad();
...
/****
	...
	$file_socio = $raiz . $rdir . "DEFINITIVO_CREDINAVIDAD.TXT";
//echo $file_socio . "<br>";
	...
	$ejec = exec($raiz . $rdir . "ejec_pvx_credinavidad definitivo 2>&1");
//	$ejec = exec($raiz . $rdir . "ejec_pvx_credinavidad definitivo");
echo "ejecutando definitivo <br>";

	$searches = $ver->planillas( $ced );
//	$afi = str_replace(",","",$afi);
...
//echo "serie: " . $fpdf . "<br>";
	if ( !empty($fpdf) ) {
		$pdff = $docr . "/pdfs/credinavidad/" . $ced . ".pdf";
...
//	$updf = JURI::base(). "media/pdf_credinavidad/" . $ced . ".pdf";
***/
...
case 'cal':
  {
	...
	$file_socio = $raiz . $rdir . "PRE_CALCULO_CREDINAVIDAD.TXT";

        $contenido = $ced . PHP_EOL . $dpm . PHP_EOL . $tip . PHP_EOL . $opc . PHP_EOL . $mon . PHP_EOL;
        $datu = $ver->datusuario( $ced );
        $contenido .= ($ver->validaemail( $ema ) ? $ema : $datu[1]) . PHP_EOL;
        $contenido .= $datu[0];

	escribir_archivo($file_socio, $contenido);

	$ejec = exec($raiz . $rdir . "ejec_pvx_credinavidad calculo 2>&1");

	$filas = $raiz . $rdir . $ced . "_CALCULO_CREDINAVIDAD.TXT";
...
default:
  if ( !empty($ced) )
  {
	$file_socio = $raiz . $rdir . "DISPONIBILIDAD_CREDINAVIDAD.TXT";

	escribir_archivo($file_socio, $ced);

	$ejec = exec($raiz . $rdir . "ejec_pvx_credinavidad disponibilidad 2>&1");

//	echo "paso... " . $s;
	$filas = $raiz . $rdir . $ced . "_DISPONIBILIDAD_CREDINAVIDAD.TXT";
