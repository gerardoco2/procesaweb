...
require_once("lib_credifuneraria.php");
$ver = new Libcredifuneraria();
...
case 'cal':
  {
	...
	$file_socio = $raiz . $rdir . "PRE_CALCULO_CREDIFUNERARIO.TXT";
	...
	$ejec = exec($raiz . $rdir . "ejec_pvx_credifunerario calculo 2>&1");

	$filas = $raiz . $rdir . $ced . "_CALCULO_CREDIFUNERARIO.TXT";
...
default:
  if ( !empty($ced) )
  {
	$file_socio = $raiz . $rdir . "DISPONIBILIDAD_CREDIFUNERARIO.TXT";

///	escribir_archivo($file_socio, $ced);

///	$ejec = exec($raiz . $rdir . "ejec_pvx_credifunerario disponibilidad 2>&1");

//	echo "paso... " . $s;
	$filas = $raiz . $rdir . $ced . "_DISPONIBILIDAD_CREDIFUNERARIO.TXT";
